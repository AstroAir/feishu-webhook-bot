...
def test_event_server_receive_event(basic_config, event_handler):
    ...
    payload = {"type": "message", "event": {"text": "Hello", "sender": {"user_id": "user123"}}}
...
def test_event_server_verification_token_valid(config_with_auth, event_handler):
    ...
    payload = {"type": "message", "token": "test_token_12345", "event": {"text": "Hello"}}

    body = json.dumps(payload, separators=(",", ":")).encode("utf-8")
    timestamp = "1234567890"
    nonce = "token-valid-test"
    message = (timestamp + nonce).encode("utf-8") + body
    digest = hmac.new(b"test_secret", message, hashlib.sha256).digest()
...
def test_event_server_verification_token_invalid(config_with_auth, event_handler):
    ...
    payload = {"type": "message", "token": "wrong_token", "event": {"text": "Hello"}}
...
def test_event_server_start(basic_config, event_handler, mocker):
    ...
    mock_server_class = mocker.patch(
        "feishu_webhook_bot.core.event_server.uvicorn.Server", return_value=mock_server
    )
...
def test_qq_event_with_bearer_token(basic_config, event_handler):
    ...
    payload = {"message_type": "private", "user_id": 987654, "message": "Hi"}
...
def test_qq_event_invalid_json(basic_config, event_handler):
    ...
    response = client.post(
        "/qq/events", content="invalid json", headers={"Content-Type": "application/json"}
    )
