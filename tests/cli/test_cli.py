...
def test_package_exports_include_ai_components():
    """Ensure top-level package exports core and AI components."""
    import feishu_webhook_bot as package

    for expected in {"FeishuBot", "BotConfig", "WebhookConfig"}:
        assert expected in package.__all__

    if package.AIAgent is not None:
        assert "AIAgent" in package.__all__


def test_package_version_fallback(monkeypatch):
    """Reload package when metadata version is unavailable."""
    import feishu_webhook_bot as package

    original_version = package.__version__
    ...
    assert restored.__version__ == original_version


def test_module_main_invokes_cli(monkeypatch):
    """Running python -m feishu_webhook_bot delegates to CLI main."""

    def fake_main(argv: list[str] | None = None) -> int:
        return 42

    monkeypatch.setattr("feishu_webhook_bot.cli.main", fake_main)

    with pytest.raises(SystemExit) as exc:
        runpy.run_module("feishu_webhook_bot.__main__", run_name="__main__")
