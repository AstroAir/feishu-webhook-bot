...
class TestMCPConfig:
    ...
    def test_mcp_config_validation_timeout(self):
        """Test MCP configuration timeout validation."""
        config = MCPConfig(enabled=True, timeout_seconds=60)
        assert config.timeout_seconds == 60
...
class TestMCPClientWithMCPSupport:
    ...
    async def test_mcp_client_start_with_invalid_server(self):
        """Test MCP client handles invalid server configuration."""
        config = MCPConfig(
            enabled=True,
            servers=[{"name": "invalid-server"}],  # Missing command and url
        )
        client = MCPClient(config)
    ...
    async def test_mcp_client_get_mcp_servers_before_start(self):
        ...
        config = MCPConfig(
            enabled=True,
            servers=[{"name": "test-server", "command": "echo", "args": "test"}],
        )
        client = MCPClient(config)
...
class TestMCPErrorHandling:
    ...
    async def test_mcp_client_without_pydantic_ai_support(self):
        """Test MCP client when pydantic-ai MCP support is not available."""
        config = MCPConfig(
            enabled=True,
            servers=[{"name": "test-server", "command": "test", "args": "args"}],
        )
        client = MCPClient(config)
...
class TestMCPClientLifecycle:
    ...
    async def test_mcp_client_server_management(self):
        ...
        config = MCPConfig(
            enabled=True,
            servers=[
                {"name": "server1", "command": "test1", "args": "args1"},
                {"name": "server2", "url": "http://localhost:3001/mcp"},
            ],
        )
        client = MCPClient(config)
...
class TestAIAgentMCPIntegration:
    ...
    async def test_ai_agent_mcp_initialization_failure_graceful(self):
        """Test agent continues working even if MCP initialization fails."""
        from feishu_webhook_bot.ai import AIAgent, AIConfig

        config = AIConfig(
            enabled=True,
            model="openai:gpt-4o",
            mcp=MCPConfig(enabled=True, servers=[{"name": "invalid"}]),
        )
