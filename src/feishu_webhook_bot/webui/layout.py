...
NAV_ITEMS = [
    {"id": "overview", "label_key": "nav.overview", "icon": "dashboard", "category_key": "nav.dashboard"},
    {"id": "general", "label_key": "nav.general", "icon": "settings", "category_key": "nav.configuration"},
    {"id": "webhooks", "label_key": "nav.webhooks", "icon": "webhook", "category_key": "nav.configuration"},
    {"id": "scheduler", "label_key": "nav.scheduler", "icon": "schedule", "category_key": "nav.configuration"},
    {"id": "plugins", "label_key": "nav.plugins", "icon": "extension", "category_key": "nav.configuration"},
    {"id": "logging", "label_key": "nav.logging", "icon": "description", "category_key": "nav.configuration"},
    {"id": "templates", "label_key": "nav.templates", "icon": "article", "category_key": "nav.configuration"},
    {"id": "notifications", "label_key": "nav.notifications", "icon": "notifications", "category_key": "nav.configuration"},
    ...
    {"id": "automation", "label_key": "nav.automation", "icon": "auto_fix_high", "category_key": "nav.features"},
    {"id": "providers", "label_key": "nav.providers", "icon": "cloud", "category_key": "nav.features"},
    {"id": "bridge", "label_key": "nav.bridge", "icon": "sync_alt", "category_key": "nav.features"},
    {"id": "messages", "label_key": "nav.messages", "icon": "message", "category_key": "nav.monitoring"},
...
def build_ui(config_path: Path) -> None:
    ...
    with ui.header(elevated=True).style("background:#1e40af").classes("px-4 py-2"):
        with ui.row().classes("w-full items-center justify-between gap-2"):
            with ui.row().classes("items-center gap-2"):
                # Mobile menu button
                ui.button(icon="menu", on_click=lambda: ui.run_javascript("toggleMobileSidebar()")).props("flat dense round color=white").classes("mobile-menu-btn")
                ui.icon("smart_toy", size="md").classes("text-white hidden sm:block")
                ui.label(t("app.title")).classes("text-base sm:text-lg font-semibold text-white truncate max-w-32 sm:max-w-none")

            # Status chip (always visible)
            status_chip = ui.chip(t("app.status.stopped"), color="grey", text_color="white").props("outline dense")

            # Desktop controls
            with ui.row().classes("header-desktop-controls"):
                ui.button(t("header.start"), on_click=on_start).props("dense unelevated color=green-8")
                ui.button(t("header.stop"), on_click=on_stop).props("dense unelevated color=red-8")
                ui.button(t("header.restart"), on_click=on_restart).props("dense unelevated color=orange-8")
                ui.separator().props("vertical").classes("mx-1 h-6 opacity-50")
                ui.button(t("header.save"), on_click=save_to_disk).props("dense unelevated color=white text-color=blue-8")
                ui.button(t("header.reset"), on_click=reload_from_disk).props("dense outline color=white")
                ui.select(
            ...
            with ui.element("div").classes("header-mobile-controls"):
                with ui.button(icon="more_vert").props("flat dense round color=white"):
                    with ui.menu().classes("bg-white"):
                        ...
                        ui.menu_item(t("header.restart"), on_click=on_restart).classes("text-orange-600")
                        ui.separator()
                        ui.menu_item(t("header.save"), on_click=save_to_disk).classes("text-blue-600")
    ...
    with ui.element("div").classes("main-container"):
        ...
        ui.element("div").classes("sidebar-overlay").on("click", lambda: ui.run_javascript("closeMobileSidebar()"))
        
        # Left sidebar
        with ui.element("div").classes("left-sidebar"):
            # Config file info
            with ui.column().classes("mb-4 pb-3 border-b border-gray-200"):
                ui.label(t("sidebar.config_file")).classes("text-xs text-gray-500 uppercase tracking-wide")
                ui.label(config_path.name).classes("text-sm font-medium text-gray-700 truncate").tooltip(str(config_path))
            ...
            with nav_container:
                ...
                for item in NAV_ITEMS:
                    ...
                    nav_row = ui.row().classes("nav-item items-center gap-2 px-2 py-1.5 rounded w-full").on("click", make_nav_click(item["id"]))
                    with nav_row:
            ...
            async def save_profile() -> None:
                with ui.dialog() as dialog, ui.card().classes("p-4"):
                    ...
                    name_input = ui.input(t("general.name")).props("outlined dense").classes("w-full")
                    with ui.row().classes("gap-2 mt-3"):
                        ui.button(t("common.cancel"), on_click=dialog.close).props("flat")
                        ui.button(t("common.save"), on_click=lambda: dialog.submit(name_input.value)).props("color=primary")
            ...
            ui.button(t("sidebar.save_profile"), on_click=save_profile).props("flat dense size=sm").classes("w-full justify-start")
            if profile_files:
                ui.select(
            ...
            ui.button(t("sidebar.export_config"), on_click=on_export).props("flat dense size=sm").classes("w-full justify-start mt-1")
...
def build_overview_page(controller: BotController, state: dict[str, Any]) -> None:
    ...
    with ui.element("div").classes("grid grid-cols-2 sm:grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-6 mb-4 sm:mb-8 w-full"):
        # Bot status card
        status = controller.status()
        with ui.card().classes("p-3 sm:p-6 bg-white border border-gray-200 rounded-xl shadow-sm hover:shadow-md transition-shadow"):
            with ui.column().classes("gap-1 sm:gap-2 w-full"):
                ...
                with ui.row().classes("items-center gap-2 sm:justify-between w-full"):
                    with ui.element("div").classes(f"w-8 h-8 sm:w-14 sm:h-14 rounded-full bg-{color}-100 flex items-center justify-center flex-shrink-0"):
                        ui.icon("power_settings_new", size="sm").classes(f"text-{color}-600 sm:text-lg")
                    ui.label(t("dashboard.bot_status")).classes("text-xs sm:text-sm text-gray-500 font-medium truncate")
                ui.label(t("app.status.running") if status["running"] else t("app.status.stopped")).classes("text-lg sm:text-2xl font-bold text-gray-800")
        ...
        with ui.card().classes("p-3 sm:p-6 bg-white border border-gray-200 rounded-xl shadow-sm hover:shadow-md transition-shadow"):
            with ui.column().classes("gap-1 sm:gap-2 w-full"):
                ...
                with ui.row().classes("items-center gap-2 sm:justify-between w-full"):
                    ...
                    ui.label(t("dashboard.scheduler")).classes("text-xs sm:text-sm text-gray-500 font-medium truncate")
                ui.label(t("dashboard.active") if status["scheduler_running"] else t("dashboard.inactive")).classes("text-lg sm:text-2xl font-bold text-gray-800")
        ...
        with ui.card().classes("p-3 sm:p-6 bg-white border border-gray-200 rounded-xl shadow-sm hover:shadow-md transition-shadow"):
            with ui.column().classes("gap-1 sm:gap-2 w-full"):
                ...
                with ui.row().classes("items-center gap-2 sm:justify-between w-full"):
                    ...
                    ui.label(t("dashboard.ai_agent")).classes("text-xs sm:text-sm text-gray-500 font-medium truncate")
                ui.label(t("dashboard.enabled") if ai_stats["enabled"] else t("dashboard.disabled")).classes("text-lg sm:text-2xl font-bold text-gray-800")
        ...
        with ui.card().classes("p-3 sm:p-6 bg-white border border-gray-200 rounded-xl shadow-sm hover:shadow-md transition-shadow"):
            with ui.column().classes("gap-1 sm:gap-2 w-full"):
                with ui.row().classes("items-center gap-2 sm:justify-between w-full"):
                    with ui.element("div").classes("w-8 h-8 sm:w-14 sm:h-14 rounded-full bg-blue-100 flex items-center justify-center flex-shrink-0"):
                        ui.icon("mail", size="sm").classes("text-blue-600 sm:text-lg")
                    ui.label(t("dashboard.messages")).classes("text-xs sm:text-sm text-gray-500 font-medium truncate")
                with ui.row().classes("items-baseline gap-1"):
                    ui.label(str(msg_stats['success'])).classes("text-lg sm:text-2xl font-bold text-gray-800")
                    ui.label(t("dashboard.sent")).classes("text-xs text-gray-400")

    # Two column layout for quick actions and plugins
    with ui.element("div").classes("grid grid-cols-1 lg:grid-cols-2 gap-4 sm:gap-6 mb-4 sm:mb-8 w-full"):
        # Quick actions card
        with ui.card().classes("p-4 sm:p-6 bg-white border border-gray-200 rounded-xl shadow-sm"):
            ui.label(t("dashboard.quick_actions")).classes("text-base sm:text-lg font-semibold text-gray-800 mb-3 sm:mb-4")
            with ui.element("div").classes("grid grid-cols-3 gap-2 sm:gap-4"):
                ...
                with ui.card().classes("p-3 sm:p-5 bg-gradient-to-br from-blue-50 to-blue-100 border-0 rounded-xl cursor-pointer hover:from-blue-100 hover:to-blue-200 transition-all").on("click", on_send_test_click):
                    with ui.column().classes("items-center gap-2 sm:gap-3"):
                        with ui.element("div").classes("w-10 h-10 sm:w-12 sm:h-12 rounded-full bg-blue-500 flex items-center justify-center"):
                            ui.icon("send", size="sm").classes("text-white")
                        ui.label(t("dashboard.send_test")).classes("text-xs sm:text-sm font-medium text-blue-700 text-center")

                with ui.card().classes("p-3 sm:p-5 bg-gradient-to-br from-green-50 to-green-100 border-0 rounded-xl cursor-pointer hover:from-green-100 hover:to-green-200 transition-all").on("click", on_reload_config_click):
                    with ui.column().classes("items-center gap-2 sm:gap-3"):
                        with ui.element("div").classes("w-10 h-10 sm:w-12 sm:h-12 rounded-full bg-green-500 flex items-center justify-center"):
                            ui.icon("refresh", size="sm").classes("text-white")
                        ui.label(t("dashboard.reload_config")).classes("text-xs sm:text-sm font-medium text-green-700 text-center")

                with ui.card().classes("p-3 sm:p-5 bg-gradient-to-br from-gray-50 to-gray-100 border-0 rounded-xl cursor-pointer hover:from-gray-100 hover:to-gray-200 transition-all").on("click", on_view_logs_click):
                    with ui.column().classes("items-center gap-2 sm:gap-3"):
                        with ui.element("div").classes("w-10 h-10 sm:w-12 sm:h-12 rounded-full bg-gray-500 flex items-center justify-center"):
                            ui.icon("terminal", size="sm").classes("text-white")
                        ui.label(t("dashboard.view_logs")).classes("text-xs sm:text-sm font-medium text-gray-700 text-center")

        # Active plugins card
        with ui.card().classes("p-4 sm:p-6 bg-white border border-gray-200 rounded-xl shadow-sm"):
            ui.label(t("dashboard.active_plugins")).classes("text-base sm:text-lg font-semibold text-gray-800 mb-3 sm:mb-4")
            status = controller.status()
            if status["plugins"]:
                with ui.row().classes("gap-2 sm:gap-3 flex-wrap"):
                    for plugin in status["plugins"]:
                        ui.chip(plugin, color="blue").classes("text-xs sm:text-sm px-2 sm:px-4 py-1 sm:py-2")
            else:
                with ui.column().classes("items-center justify-center py-4 sm:py-8"):
                    ...
                    ui.label(t("dashboard.no_plugins")).classes("text-sm sm:text-base text-gray-400")

    # Config summary - full width with better stats display
    with ui.card().classes("p-4 sm:p-6 bg-white border border-gray-200 rounded-xl shadow-sm w-full"):
        ui.label(t("dashboard.config_summary")).classes("text-base sm:text-lg font-semibold text-gray-800 mb-4 sm:mb-6")
        ...
        with ui.element("div").classes("grid grid-cols-3 gap-2 sm:gap-8"):
            with ui.column().classes("items-center gap-1 sm:gap-2 p-2 sm:p-4 bg-blue-50 rounded-xl"):
                ui.label(str(len(webhooks))).classes("text-2xl sm:text-4xl font-bold text-blue-600")
                ui.label(t("nav.webhooks")).classes("text-xs sm:text-sm text-blue-700 font-medium text-center")
            with ui.column().classes("items-center gap-1 sm:gap-2 p-2 sm:p-4 bg-green-50 rounded-xl"):
                ui.label(str(len(templates))).classes("text-2xl sm:text-4xl font-bold text-green-600")
                ui.label(t("nav.templates")).classes("text-xs sm:text-sm text-green-700 font-medium text-center")
            with ui.column().classes("items-center gap-1 sm:gap-2 p-2 sm:p-4 bg-orange-50 rounded-xl"):
                ui.label(str(len(notifications))).classes("text-2xl sm:text-4xl font-bold text-orange-600")
                ui.label(t("nav.notifications")).classes("text-xs sm:text-sm text-orange-700 font-medium text-center")
