...
def build_events_page(controller: BotController, state: dict[str, Any] | None = None) -> None:
    ...
    with ui.element("div").classes("grid grid-cols-2 sm:grid-cols-4 gap-3 sm:gap-4 mb-4 sm:mb-6 w-full"):
        ...
        with ui.card().classes("p-3 sm:p-4 bg-blue-50 border border-blue-100 rounded-xl"):
            with ui.column().classes("items-center gap-1"):
                ui.icon("dns" if running else "dns", size="md").classes(f"text-{'green' if running else 'gray'}-600")
        ...
        with ui.card().classes("p-3 sm:p-4 bg-green-50 border border-green-100 rounded-xl"):
            with ui.column().classes("items-center gap-1"):
                ui.label(str(len(recent_events))).classes("text-xl sm:text-2xl font-bold text-green-600")
                ui.label(t("events.recent_count")).classes("text-xs sm:text-sm text-green-700 text-center")

        # Host
        with ui.card().classes("p-3 sm:p-4 bg-purple-50 border border-purple-100 rounded-xl"):
            with ui.column().classes("items-center gap-1"):
                ui.label(event_status.get("host", "N/A")).classes("text-sm sm:text-base font-bold text-purple-600 truncate")
        ...
        with ui.card().classes("p-3 sm:p-4 bg-orange-50 border border-orange-100 rounded-xl"):
            with ui.column().classes("items-center gap-1"):
                ui.label(str(event_status.get("port", "N/A"))).classes("text-xl sm:text-2xl font-bold text-orange-600")
    ...
    with ui.element("div").classes("grid grid-cols-1 lg:grid-cols-2 gap-4 sm:gap-6 mb-4 sm:mb-6 w-full"):
        ...
        with ui.card().classes("p-4 sm:p-6 bg-white border border-gray-200 rounded-xl shadow-sm"):
            ui.label(t("events.server_status")).classes("text-base sm:text-lg font-semibold text-gray-800 mb-3 sm:mb-4")
            with ui.row().classes("items-center gap-6"):
                color = "green" if event_status["running"] else "gray"
                with ui.element("div").classes(f"w-16 h-16 rounded-full bg-{color}-100 flex items-center justify-center"):
                    ui.icon("dns", size="xl").classes(f"text-{color}-600")
                with ui.column().classes("gap-2"):
                    ui.chip(
                        t("app.status.running") if event_status["running"] else t("app.status.stopped"),
                        color="green" if event_status["running"] else "grey"
                    ).classes("text-sm")
                    ui.label(f"{t('events.host')}: {event_status['host']}").classes("text-sm text-gray-600")
                    ui.label(f"{t('events.port')}: {event_status['port']}").classes("text-sm text-gray-600")

        # Server controls card
        with ui.card().classes("p-4 sm:p-6 bg-white border border-gray-200 rounded-xl shadow-sm"):
            ui.label(t("events.server_controls")).classes("text-base sm:text-lg font-semibold text-gray-800 mb-3 sm:mb-4")
            ...
            with ui.row().classes("gap-4"):
                ui.button(t("events.start"), on_click=on_start_server, icon="play_arrow").props("color=green size=lg")
                ui.button(t("events.stop"), on_click=on_stop_server, icon="stop").props("color=red size=lg")
    ...
    with ui.element("div").classes("grid grid-cols-1 lg:grid-cols-2 gap-4 sm:gap-6 w-full"):
        ...
        with ui.card().classes("p-4 sm:p-6 bg-white border border-gray-200 rounded-xl shadow-sm"):
            with ui.row().classes("items-center justify-between mb-3 sm:mb-4"):
                ui.label(t("events.recent_events")).classes("text-base sm:text-lg font-semibold text-gray-800")
                ui.button(icon="refresh", on_click=lambda: rebuild_events()).props("flat round dense")
        ...
        # Webhook test card
        with ui.card().classes("p-4 sm:p-6 bg-white border border-gray-200 rounded-xl shadow-sm"):
            ui.label(t("events.webhook_test")).classes("text-base sm:text-lg font-semibold text-gray-800 mb-3 sm:mb-4")
            test_payload = ui.textarea(t("events.test_payload")).classes("w-full").props("outlined auto-grow rows=5")
            ...
            ui.button(t("events.send_test"), on_click=on_send_webhook, icon="send").props("color=primary").classes("mt-4")
