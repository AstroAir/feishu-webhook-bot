...
def build_status_page(state: dict[str, Any], controller: BotController) -> None:
    ...
    with ui.element("div").classes(
        ...
        with ui.card().classes("p-3 sm:p-4 bg-blue-50 border border-blue-100 rounded-xl"):
            with ui.column().classes("items-center gap-1"):
                ui.icon("power_settings_new", size="md").classes(
                    f"text-{'green' if running else 'gray'}-600"
                )
                ui.label(t("status.bot_running") if running else t("status.bot_stopped")).classes(
        ...
        with ui.card().classes("p-3 sm:p-4 bg-green-50 border border-green-100 rounded-xl"):
            with ui.column().classes("items-center gap-1"):
                ui.icon("schedule", size="md").classes(
                    f"text-{'green' if scheduler_running else 'gray'}-600"
                )
                ui.label(
                    t("status.scheduler_active")
                    if scheduler_running
                    else t("status.scheduler_inactive")
                ).classes("text-xs sm:text-sm text-green-700 text-center")
        ...
        with ui.card().classes("p-3 sm:p-4 bg-purple-50 border border-purple-100 rounded-xl"):
            with ui.column().classes("items-center gap-1"):
                ...
                ui.label(t("status.plugins_loaded")).classes(
        ...
        with ui.card().classes("p-3 sm:p-4 bg-orange-50 border border-orange-100 rounded-xl"):
            with ui.column().classes("items-center gap-1"):
                ui.label(str(len(cfg_webhooks))).classes(
                    "text-xl sm:text-2xl font-bold text-orange-600"
                )
                ui.label(t("status.webhooks_configured")).classes(
    ...
    with ui.card().classes("w-full p-6 bg-white border border-gray-200 rounded-xl shadow-sm mt-6"):
        ...
        msg_input = (
            ui.input(t("status.message")).props("outlined dense clearable").classes("w-full")
        )
            ...
            with jobs_container:
                ...
                for job in jobs:
                    with ui.row().classes("items-center gap-3 p-2 bg-gray-50 rounded"):
                        ...
                        ui.label(f"{t('tasks.next_run')}: {job.next_run_time}").classes(
                        ...
                        ui.button(t("status.remove"), on_click=_remove).props(
                            "dense size=sm color=red"
                        )

        with ui.row().classes("gap-2"):
            ui.button(t("status.refresh_status"), on_click=refresh_status).props(
                "flat color=primary"
            )
