...
def build_templates_page(state: dict[str, Any]) -> None:
    ...
    with ui.element("div").classes(
        ...
        with ui.card().classes("p-3 sm:p-4 bg-blue-50 border border-blue-100 rounded-xl"):
            with ui.column().classes("items-center gap-1"):
                ...
                ui.label(t("templates.total")).classes(
        ...
        with ui.card().classes("p-3 sm:p-4 bg-orange-50 border border-orange-100 rounded-xl"):
            with ui.column().classes("items-center gap-1"):
                ...
                ui.label(t("templates.other")).classes(
                    "text-xs sm:text-sm text-orange-700 text-center"
                )

    # Template type reference
    with (
        ui.expansion(t("templates.type_reference"), icon="help_outline").classes(
            "w-full mb-4 bg-white rounded-xl border border-gray-200"
        ),
        ui.element("div").classes("grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 p-4"),
    ):
    ...
    with (
        ...
        ui.element("div").classes("grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-2 sm:gap-3"),
    ):
        ...
        ui.button(
            "文本通知",
            on_click=lambda: add_preset("text", "text-notify", "通知: {message}"),
            icon="text_fields",
        ).props("outline dense").classes("w-full")
        ui.button(
            "Markdown",
            on_click=lambda: add_preset("markdown", "md-template", "**标题**\n\n{content}"),
            icon="code",
        ).props("outline dense").classes("w-full")
        ui.button(
            "卡片消息",
            on_click=lambda: add_preset(
                "card", "card-template", '{"header":{"title":"标题"},"elements":[]}'
            ),
            icon="dashboard",
        ).props("outline dense").classes("w-full")
        ui.button(
            "富文本",
            on_click=lambda: add_preset("post", "post-template", '{"title":"标题","content":[]}'),
            icon="article",
        ).props("outline dense").classes("w-full")
        ui.button(
            "交互式",
            on_click=lambda: add_preset(
                "interactive", "interactive-template", '{"elements":[],"actions":[]}'
            ),
            icon="touch_app",
        ).props("outline dense").classes("w-full")
        ui.button(
            "JSON",
            on_click=lambda: add_preset(
                "json", "json-template", '{"msg_type":"text","content":{}}'
            ),
            icon="data_object",
        ).props("outline dense").classes("w-full")
