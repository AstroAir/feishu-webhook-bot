...
def build_ai_dashboard_page(state: dict[str, Any], controller: BotController) -> None:
    ...
    with ui.element("div").classes("grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4 mb-4 sm:mb-6 w-full"):
        # Status card
        with ui.card().classes("p-3 sm:p-5 bg-white border border-gray-200 rounded-xl shadow-sm"):
            with ui.column().classes("gap-1 sm:gap-2 w-full"):
                ...
                with ui.row().classes("items-center gap-2"):
                    with ui.element("div").classes(f"w-8 h-8 sm:w-12 sm:h-12 rounded-full bg-{color}-100 flex items-center justify-center flex-shrink-0"):
                        ui.icon("check_circle" if ai_stats["enabled"] else "cancel", size="sm").classes(f"text-{color}-600")
                    ui.label(t("ai.status")).classes("text-xs sm:text-sm text-gray-500 truncate")
                ui.label(t("dashboard.enabled") if ai_stats["enabled"] else t("dashboard.disabled")).classes("text-base sm:text-xl font-bold text-gray-800")

        # Requests card
        with ui.card().classes("p-3 sm:p-5 bg-white border border-gray-200 rounded-xl shadow-sm"):
            with ui.column().classes("gap-1 sm:gap-2 w-full"):
                with ui.row().classes("items-center gap-2"):
                    with ui.element("div").classes("w-8 h-8 sm:w-12 sm:h-12 rounded-full bg-blue-100 flex items-center justify-center flex-shrink-0"):
                ...
                ui.label(str(ai_stats["requests"])).classes("text-base sm:text-xl font-bold text-gray-800")

        # Success rate card
        with ui.card().classes("p-3 sm:p-5 bg-white border border-gray-200 rounded-xl shadow-sm"):
            with ui.column().classes("gap-1 sm:gap-2 w-full"):
                with ui.row().classes("items-center gap-2"):
                    with ui.element("div").classes("w-8 h-8 sm:w-12 sm:h-12 rounded-full bg-green-100 flex items-center justify-center flex-shrink-0"):
                        ui.icon("trending_up", size="sm").classes("text-green-600")
                    ui.label(t("ai.success_rate")).classes("text-xs sm:text-sm text-gray-500 truncate")
                ui.label(f"{ai_stats['success_rate']:.1%}").classes("text-base sm:text-xl font-bold text-gray-800")

        # Tokens card
        with ui.card().classes("p-3 sm:p-5 bg-white border border-gray-200 rounded-xl shadow-sm"):
            with ui.column().classes("gap-1 sm:gap-2 w-full"):
                with ui.row().classes("items-center gap-2"):
                    with ui.element("div").classes("w-8 h-8 sm:w-12 sm:h-12 rounded-full bg-purple-100 flex items-center justify-center flex-shrink-0"):
                        ui.icon("token", size="sm").classes("text-purple-600")
                    ui.label(t("ai.tokens_used")).classes("text-xs sm:text-sm text-gray-500 truncate")
                ui.label(str(ai_stats["tokens_used"])).classes("text-base sm:text-xl font-bold text-gray-800")
    ...
    with ui.element("div").classes("grid grid-cols-1 lg:grid-cols-2 gap-4 sm:gap-6 mb-4 sm:mb-6 w-full"):
        ...
        with ui.card().classes("p-4 sm:p-6 bg-white border border-gray-200 rounded-xl shadow-sm"):
            ui.label(t("ai.basic_settings")).classes("text-base sm:text-lg font-semibold text-gray-800 mb-3 sm:mb-4")
            with ui.column().classes("gap-4 w-full"):
                ...
                ui.input(t("ai.api_key")).bind_value(cfg_ai, "api_key").props("type=password outlined clearable").classes("w-full")
        ...
        with ui.card().classes("p-6 bg-white border border-gray-200 rounded-xl shadow-sm"):
            ...
            with ui.column().classes("gap-4 w-full"):
                ...
                ui.input(t("ai.base_url")).bind_value(provider_cfg, "base_url").props("outlined clearable").classes("w-full").tooltip(t("ai.base_url_hint"))
                ui.input(t("ai.organization_id")).bind_value(provider_cfg, "organization_id").props("outlined clearable").classes("w-full")
    ...
    with ui.element("div").classes("grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6 w-full"):
        ...
        with ui.card().classes("p-6 bg-white border border-gray-200 rounded-xl shadow-sm"):
            ...
            with ui.column().classes("gap-4 w-full"):
                with ui.row().classes("items-center gap-4 w-full"):
                    ...
                    ui.slider(min=0.0, max=2.0, step=0.1, value=cfg_ai.get("temperature", 0.7)).bind_value(cfg_ai, "temperature").classes("flex-grow")
                    ui.label().bind_text_from(cfg_ai, "temperature", lambda v: f"{v:.1f}" if v else "0.7").classes("w-12 text-sm")
                ui.number(t("ai.max_tokens"), min=1, max=128000).bind_value(cfg_ai, "max_tokens").props("outlined").classes("w-full")
                ui.number(t("ai.max_conversation_turns"), min=1, max=100).bind_value(cfg_ai, "max_conversation_turns").props("outlined").classes("w-full")
                ui.number(t("ai.conversation_timeout"), min=1, max=1440).bind_value(cfg_ai, "conversation_timeout_minutes").props("outlined").classes("w-full")
        ...
        with ui.card().classes("p-6 bg-white border border-gray-200 rounded-xl shadow-sm"):
            ...
            with ui.column().classes("gap-3 w-full"):
                ...
                ui.switch(t("ai.streaming_enabled")).bind_value(cfg_ai.setdefault("streaming", {}), "enabled")
                ui.number(t("ai.max_retries"), min=1, max=10).bind_value(cfg_ai, "max_retries").props("outlined").classes("w-full")
    ...
    with ui.card().classes("w-full p-6 bg-white border border-gray-200 rounded-xl shadow-sm mb-6"):
        ...
        ui.textarea(t("ai.system_prompt_hint")).bind_value(cfg_ai, "system_prompt").props("outlined auto-grow rows=4").classes("w-full")
                ...
                if not fallback_list:
                    ...
                    for idx, model in enumerate(fallback_list):
                        with ui.row().classes("items-center gap-2"):
                            ...
                            ui.button(icon="close", on_click=remove_fallback).props("flat round dense size=sm color=red")

        rebuild_fallback_list()

        with ui.row().classes("items-center gap-2 mt-3"):
            new_fallback = ui.select(AVAILABLE_MODELS, label=t("ai.add_fallback")).props("outlined dense").classes("w-64")
    ...
    with ui.card().classes("w-full p-6 bg-white border border-gray-200 rounded-xl shadow-sm mb-6"):
        ...
        with ui.row().classes("items-center justify-between mb-4"):
            ...
            ui.number(t("ai.mcp_timeout"), min=1, max=300).bind_value(mcp_cfg, "timeout_seconds").props("outlined dense").classes("w-48")
                        ...
                        with ui.card().classes("w-full p-4 bg-gray-50 border border-gray-100 rounded-lg"):
                            with ui.row().classes("items-start justify-between"):
                                with ui.column().classes("gap-2 flex-grow"):
                                    ui.input(t("ai.mcp_server_name")).bind_value(server, "name").props("outlined dense").classes("w-full")
                                    ui.input(t("ai.mcp_server_command")).bind_value(server, "command").props("outlined dense").classes("w-full")
                                    ui.input(t("ai.mcp_server_args")).bind_value(server, "args").props("outlined dense").classes("w-full").tooltip(t("ai.mcp_args_hint"))

                                def remove_server(i: int = idx) -> None:
                                    mcp_servers.pop(i)
                                    rebuild_mcp_servers()

                                ui.button(icon="delete", on_click=remove_server).props("flat round color=red")
        ...
        ui.button(t("ai.add_mcp_server"), on_click=add_mcp_server, icon="add").props("outline color=primary").classes("mt-3")
    ...
    with ui.card().classes("w-full p-6 bg-white border border-gray-200 rounded-xl shadow-sm mb-6"):
        ...
        with ui.column().classes("gap-4 w-full"):
            ...
            ui.input(t("ai.persistence_database")).bind_value(persistence_cfg, "database_url").props("outlined").classes("w-full").tooltip(t("ai.persistence_database_hint"))
            with ui.row().classes("gap-4 w-full"):
                ui.number(t("ai.max_history_days"), min=1, max=365).bind_value(persistence_cfg, "max_history_days").props("outlined").classes("flex-1")
                ui.number(t("ai.cleanup_interval"), min=1, max=168).bind_value(persistence_cfg, "cleanup_interval_hours").props("outlined").classes("flex-1")
    ...
    with ui.card().classes("w-full p-6 bg-white border border-gray-200 rounded-xl shadow-sm mb-6"):
        ...
        with ui.column().classes("gap-4 w-full"):
            ...
            ui.number(t("ai.max_agents"), min=1, max=10).bind_value(multi_agent_cfg, "max_agents").props("outlined").classes("w-full")
    ...
    with ui.card().classes("w-full p-6 bg-white border border-gray-200 rounded-xl shadow-sm mb-6"):
        with ui.column().classes("gap-4 w-full"):
            ui.switch(t("ai.web_search_enabled")).bind_value(cfg_ai, "web_search_enabled")
            ui.number(t("ai.web_search_max_results"), min=1, max=20).bind_value(cfg_ai, "web_search_max_results").props("outlined").classes("w-full")
    ...
    with ui.card().classes("w-full p-6 bg-white border border-gray-200 rounded-xl shadow-sm mb-6"):
        with ui.column().classes("gap-4 w-full"):
            ui.switch(t("ai.output_validators_enabled")).bind_value(cfg_ai, "output_validators_enabled")
            ui.switch(t("ai.retry_on_validation_error")).bind_value(cfg_ai, "retry_on_validation_error")
                    ...
                    with ui.row().classes("gap-2 flex-wrap"):
                        for idx, model in enumerate(available_models_list):
                            with ui.row().classes("items-center gap-1"):
                                ...
                                ui.button(icon="close", on_click=remove_model).props("flat round dense size=sm color=red")

        rebuild_available_models()

        with ui.row().classes("items-center gap-2 mt-3"):
            new_model = ui.select(AVAILABLE_MODELS, label=t("ai.add_available_model")).props("outlined dense").classes("w-64")
    ...
    with ui.element("div").classes("grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6 w-full"):
        ...
        with ui.card().classes("p-6 bg-white border border-gray-200 rounded-xl shadow-sm"):
            ...
            provider_cfg = cfg_ai.setdefault("provider_config", {})
            with ui.column().classes("gap-4 w-full"):
                ui.number(t("ai.request_timeout"), min=1, max=300).bind_value(provider_cfg, "timeout").props("outlined").classes("w-full")
                ui.number(t("ai.provider_max_retries"), min=0, max=10).bind_value(provider_cfg, "max_retries").props("outlined").classes("w-full")
                ui.input(t("ai.api_version")).bind_value(provider_cfg, "api_version").props("outlined clearable").classes("w-full")
        ...
        with ui.card().classes("p-6 bg-white border border-gray-200 rounded-xl shadow-sm"):
            ...
            with ui.column().classes("gap-4 w-full"):
                ...
                ui.number(t("ai.debounce_ms"), min=0, max=1000).bind_value(streaming_cfg, "debounce_ms").props("outlined").classes("w-full")
    ...
    with ui.card().classes("w-full p-6 bg-white border border-gray-200 rounded-xl shadow-sm mb-6"):
        ...
        with ui.column().classes("gap-4 w-full"):
            ui.input(t("ai.command_prefix")).bind_value(command_cfg, "prefix").props("outlined").classes("w-48").tooltip(t("ai.command_prefix_hint"))
    ...
    with ui.card().classes("w-full p-6 bg-white border border-gray-200 rounded-xl shadow-sm mb-6"):
                ...
                if not headers_list:
                    ...
                    for idx, header in enumerate(headers_list):
                        with ui.row().classes("items-center gap-2 w-full"):
                            ui.input(t("ai.header_key")).bind_value(header, "key").props("outlined dense").classes("flex-1")
                            ui.input(t("ai.header_value")).bind_value(header, "value").props("outlined dense").classes("flex-1")

                            def remove_header(i: int = idx) -> None:
                                headers_list.pop(i)
                                rebuild_headers()

                            ui.button(icon="delete", on_click=remove_header).props("flat round dense color=red")
        ...
        ui.button(t("ai.add_header"), on_click=add_header, icon="add").props("outline").classes("mt-3")
    ...
    with ui.element("div").classes("grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6 w-full"):
        ...
        with ui.card().classes("p-6 bg-white border border-gray-200 rounded-xl shadow-sm"):
            ...
            test_input = ui.textarea(placeholder=t("ai.test_placeholder")).classes("w-full").props("outlined auto-grow rows=6")
            response_area = ui.textarea(t("ai.response")).classes("w-full mt-4").props("outlined readonly auto-grow rows=6")
            ...
            with ui.row().classes("gap-2 mt-4"):
                ...
                ui.button(t("ai.clear"), on_click=lambda: (setattr(test_input, "value", ""), setattr(response_area, "value", "")), icon="clear").props("outline")
        ...
        with ui.card().classes("p-6 bg-white border border-gray-200 rounded-xl shadow-sm"):
            ...
            with ui.row().classes("items-center justify-between mb-4"):
                ...
                ui.button(icon="refresh", on_click=lambda: rebuild_tools()).props("flat round dense")
            ...
            with ui.row().classes("items-center justify-between mb-4"):
                ...
                ui.button(icon="refresh", on_click=lambda: rebuild_mcp_status()).props("flat round dense")
        ...
        def rebuild_perf_stats() -> None:
            ...
            with perf_stats_container:
                ...
                with ui.row().classes("mt-4 gap-4"):
                    ui.label(f"{t('ai.input_tokens')}: {stats.get('total_input_tokens', 0):,}").classes(
                        "text-sm text-gray-600"
                    )
                    ui.label(f"{t('ai.output_tokens')}: {stats.get('total_output_tokens', 0):,}").classes(
                        "text-sm text-gray-600"
                    )
                    ui.label(
            ...
            with conv_container:
                ...
                for conv in conversations:
                    with ui.row().classes(
                        ...
                        ui.label(conv["user_id"][:20] + "..." if len(conv["user_id"]) > 20 else conv["user_id"]).classes(
                            "flex-1 text-sm"
                        )
                        ...
                        with ui.row().classes("w-24 justify-center gap-1"):
                            async def clear_conv(uid: str = conv["user_id"]) -> None:
                                ...
                                if result.get("success"):
                                    ...
                                    ui.notify(result.get("error", t("common.error")), type="negative")
        ...
        def rebuild_tool_details() -> None:
            ...
            with tool_details_container:
                ...
                with ui.row().classes("gap-4 mb-4"):
                    ...
                    ui.label(
                        f"{t('ai.total_usage')}: {tool_stats.get('total_usage', 0)}"
                    ).classes("text-sm bg-green-100 px-3 py-1 rounded-full")
                    ...
                    for tool in tools:
                        with ui.card().classes("p-4 border border-gray-200 rounded-lg"):
                            ...
                            with ui.row().classes("gap-2 text-xs"):
                                ui.chip(tool.get("category", "general"), color="blue").props("dense")
                                ui.label(f"{t('ai.usage')}: {tool.get('usage_count', 0)}").classes(
                                    "text-gray-500"
                                )
                                if tool.get("error_count", 0) > 0:
                                    ui.label(f"{t('ai.errors')}: {tool.get('error_count', 0)}").classes(
                                        "text-red-500"
                                    )
                    ...
                    with ui.row().classes("gap-2 flex-wrap"):
                        for agent in agents:
                            with ui.chip(color="purple").classes("px-3 py-1"):
                                ui.label(f"{agent.get('name', 'Unknown')} ({agent.get('role', 'N/A')})")
