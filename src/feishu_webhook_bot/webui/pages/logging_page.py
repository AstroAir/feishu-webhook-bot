...
def build_logging_page(state: dict[str, Any], controller: BotController) -> None:
    ...
    with ui.element("div").classes(
        ...
        for level, _desc in LOG_LEVEL_INFO.items():
            is_current = level == current_level
            color = level_colors.get(level, "gray")
            with ui.card().classes(
                f"p-3 sm:p-4 {'bg-' + color + '-50 border-' + color + '-200' if is_current else 'bg-gray-50 border-gray-100'} border rounded-xl cursor-pointer hover:shadow-md transition-shadow"
            ):
                with ui.column().classes("items-center gap-1"):
                    ui.label(level).classes(
                        f"text-sm sm:text-base font-bold {'text-' + color + '-600' if is_current else 'text-gray-600'}"
                    )
                    if is_current:
    ...
    with ui.column().classes("w-full mb-3 sm:mb-4"):
        ui.label(t("logging.basic_settings")).classes(
            "text-lg sm:text-xl font-semibold text-gray-800"
        )
        ui.label(t("logging.basic_settings_desc")).classes(
    ...
    with (
        ...
        ui.element("div").classes("grid grid-cols-1 lg:grid-cols-2 gap-4 sm:gap-6"),
    ):
        # Log level selection
        with ui.column().classes("gap-3"):
            level_select = (
                ui.select(
                    ["DEBUG", "INFO", "WARNING", "ERROR", "CRITICAL"], label=t("logging.level")
                )
                .bind_value(log_cfg, "level")
                .classes("w-full")
                .props("outlined")
            )
            ui.label(LOG_LEVEL_INFO.get(log_cfg.get("level", "INFO"), "")).classes(
                "text-xs sm:text-sm text-gray-500"
            )

        # Log format selection
        with ui.column().classes("gap-3"):
            ui.input(t("logging.format")).bind_value(log_cfg, "format").classes("w-full").props(
                "outlined"
            )
    ...
    with ui.column().classes("w-full mb-3 sm:mb-4"):
        ui.label(t("logging.file_settings")).classes(
            "text-lg sm:text-xl font-semibold text-gray-800"
        )
    ...
    with (
        ...
        ui.element("div").classes("grid grid-cols-1 lg:grid-cols-3 gap-4 sm:gap-6"),
    ):
        ui.input(t("logging.file")).bind_value(log_cfg, "log_file").classes("w-full").props(
            "outlined"
        ).tooltip(t("logging.file_hint"))
        ui.number(t("logging.max_bytes"), min=1024, max=104857600, step=1048576).bind_value(
            log_cfg, "max_bytes"
        ).classes("w-full").props("outlined")
        ui.number(t("logging.backup_count"), min=0, max=100).bind_value(
            log_cfg, "backup_count"
        ).classes("w-full").props("outlined")

    # Runtime controls
    with ui.column().classes("w-full mb-3 sm:mb-4"):
        ui.label(t("logging.runtime_controls")).classes(
            "text-lg sm:text-xl font-semibold text-gray-800"
        )
        ui.label(t("logging.runtime_controls_desc")).classes(
            "text-sm sm:text-base text-gray-500 mt-1"
        )

    with (
        ui.card().classes("w-full p-4 sm:p-6 bg-white border border-gray-200 rounded-xl shadow-sm"),
        ui.row().classes("items-center gap-3 flex-wrap"),
    ):
        ui.button(
            t("logging.apply_level"),
            on_click=lambda: (
                controller.set_runtime_log_level(level_select.value or "INFO"),
        ...
        ui.button(
            t("logging.set_debug"),
            on_click=lambda: (
                controller.set_runtime_log_level("DEBUG"),
                ui.notify("Set to DEBUG", type="info"),
            ),
            icon="bug_report",
        ).props("outline")
        ui.button(
            t("logging.set_info"),
            on_click=lambda: (
                controller.set_runtime_log_level("INFO"),
                ui.notify("Set to INFO", type="info"),
            ),
            icon="info",
        ).props("outline")
