...
def build_notifications_page(state: dict[str, Any]) -> None:
    ...
    with ui.element("div").classes(
        ...
        with ui.card().classes("p-3 sm:p-4 bg-blue-50 border border-blue-100 rounded-xl"):
            with ui.column().classes("items-center gap-1"):
                ui.label(str(len(notification_list))).classes(
                    "text-xl sm:text-2xl font-bold text-blue-600"
                )
                ui.label(t("notifications.total")).classes(
        ...
        with ui.card().classes("p-3 sm:p-4 bg-green-50 border border-green-100 rounded-xl"):
            with ui.column().classes("items-center gap-1"):
                ...
                ui.label(t("notifications.active")).classes(
        ...
        with ui.card().classes("p-3 sm:p-4 bg-purple-50 border border-purple-100 rounded-xl"):
            with ui.column().classes("items-center gap-1"):
                ui.label(str(with_conditions)).classes(
                    "text-xl sm:text-2xl font-bold text-purple-600"
                )
                ui.label(t("notifications.with_conditions")).classes(
        ...
        with ui.card().classes("p-3 sm:p-4 bg-orange-50 border border-orange-100 rounded-xl"):
            with ui.column().classes("items-center gap-1"):
                ui.label(str(len(template_list))).classes(
                    "text-xl sm:text-2xl font-bold text-orange-600"
                )
                ui.label(t("notifications.templates")).classes(
                    "text-xs sm:text-sm text-orange-700 text-center"
                )

    # Trigger reference
    with (
        ui.expansion(t("notifications.trigger_reference"), icon="help_outline").classes(
            "w-full mb-4 bg-white rounded-xl border border-gray-200"
        ),
        ui.element("div").classes("grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-2 p-4"),
    ):
    ...
    with ui.column().classes("w-full mt-4 sm:mt-6 mb-3 sm:mb-4"):
        ui.label(t("notifications.quick_add")).classes(
            "text-lg sm:text-xl font-semibold text-gray-800"
        )
        ui.label(t("notifications.quick_add_desc")).classes(
    ...
    with (
        ...
        ui.element("div").classes("grid grid-cols-2 sm:grid-cols-4 gap-2 sm:gap-3"),
    ):
        ...
        ui.button(
            "消息通知",
            on_click=lambda: add_preset_notification("msg-notify", "message.received"),
            icon="message",
        ).props("outline dense").classes("w-full")
        ui.button(
            "错误告警",
            on_click=lambda: add_preset_notification("error-alert", "event.error"),
            icon="error",
        ).props("outline dense").classes("w-full")
        ui.button(
            "任务完成",
            on_click=lambda: add_preset_notification("task-done", "task.completed"),
            icon="check_circle",
        ).props("outline dense").classes("w-full")
        ui.button(
            "定时触发",
            on_click=lambda: add_preset_notification("schedule-notify", "schedule.triggered"),
            icon="schedule",
        ).props("outline dense").classes("w-full")
