...
def build_auth_page(controller: BotController, state: dict[str, Any] | None = None) -> None:
    ...
    with ui.element("div").classes(
        ...
        with ui.card().classes("p-3 sm:p-4 bg-blue-50 border border-blue-100 rounded-xl"):
            with ui.column().classes("items-center gap-1"):
                ...
                ui.label(t("auth.total_users")).classes(
        ...
        with ui.card().classes("p-3 sm:p-4 bg-green-50 border border-green-100 rounded-xl"):
            with ui.column().classes("items-center gap-1"):
                ...
                ui.label(t("auth.active_users")).classes(
        ...
        with ui.card().classes("p-3 sm:p-4 bg-red-50 border border-red-100 rounded-xl"):
            with ui.column().classes("items-center gap-1"):
                ...
                ui.label(t("auth.locked_users")).classes(
        ...
        with ui.card().classes("p-3 sm:p-4 bg-purple-50 border border-purple-100 rounded-xl"):
            with ui.column().classes("items-center gap-1"):
                ui.icon("check_circle" if auth_enabled else "cancel", size="md").classes(
                    f"text-{'green' if auth_enabled else 'gray'}-600"
                )
                ui.label(t("auth.auth_status")).classes(
    ...
    if state is not None:
        ...
        with ui.column().classes("w-full mb-3 sm:mb-4"):
            ui.label(t("auth.configuration")).classes(
                "text-lg sm:text-xl font-semibold text-gray-800"
            )
            ui.label(t("auth.configuration_desc")).classes(
        ...
        with ui.element("div").classes(
            ...
            with ui.card().classes(
                ...
                ui.label(t("auth.basic_settings")).classes(
                    "text-base sm:text-lg font-semibold text-gray-800 mb-3 sm:mb-4"
                )
                with ui.column().classes("gap-4 w-full"):
                    ui.switch(t("auth.enabled")).bind_value(cfg_auth, "enabled")
                    ui.input(t("auth.database_url")).bind_value(cfg_auth, "database_url").props(
                        "outlined"
                    ).classes("w-full").tooltip(t("auth.database_url_hint"))
                    ui.input(t("auth.jwt_secret")).bind_value(cfg_auth, "jwt_secret_key").props(
            ...
            with ui.card().classes(
                ...
                ui.label(t("auth.security_settings")).classes(
                    "text-base sm:text-lg font-semibold text-gray-800 mb-3 sm:mb-4"
                )
                with ui.column().classes("gap-4 w-full"):
                    ui.number(t("auth.token_expire"), min=1, max=1440).bind_value(
                        cfg_auth, "access_token_expire_minutes"
                    ).props("outlined").classes("w-full")
                    ui.number(t("auth.max_failed_attempts"), min=1, max=20).bind_value(
                        cfg_auth, "max_failed_attempts"
                    ).props("outlined").classes("w-full")
                    ui.number(t("auth.lockout_duration"), min=1, max=1440).bind_value(
                        cfg_auth, "lockout_duration_minutes"
                    ).props("outlined").classes("w-full")
                    ui.switch(t("auth.require_email_verification")).bind_value(
                        cfg_auth, "require_email_verification"
                    )

    # User management section
    with ui.column().classes("w-full mb-3 sm:mb-4"):
        ui.label(t("auth.user_management")).classes(
            "text-lg sm:text-xl font-semibold text-gray-800"
        )
    ...
    with ui.element("div").classes("grid grid-cols-1 lg:grid-cols-3 gap-4 sm:gap-6 w-full"):
        ...
        with ui.card().classes(
            "p-4 sm:p-6 bg-white border border-gray-200 rounded-xl shadow-sm lg:col-span-2"
        ):
            with ui.row().classes("items-center justify-between mb-3 sm:mb-4"):
                ui.label(t("auth.user_management")).classes(
                    "text-base sm:text-lg font-semibold text-gray-800"
                )
                ui.button(icon="refresh", on_click=lambda: rebuild_users()).props(
                    "flat round dense"
                )
            ...
            def rebuild_users() -> None:
                ...
                with users_container:
                    ...
                    for user in users:
                        with ui.card().classes(
                            "w-full p-4 bg-gray-50 border border-gray-100 rounded-lg hover:bg-gray-100 transition-colors"
                        ):
                            with ui.row().classes("w-full items-center justify-between"):
                                with ui.row().classes("items-center gap-4"):
                                    ...
                                    with ui.column().classes("gap-0"):
                                        ui.label(user.get("username", "N/A")).classes(
                                            "font-semibold text-gray-800"
                                        )
                                        ui.label(user.get("email", "N/A")).classes(
                                            "text-sm text-gray-500"
                                        )
                                with ui.row().classes("items-center gap-3"):
                                    status_color = (
                                        "green" if user.get("status") == "Active" else "red"
                                    )
                                    ...
                                    ui.button(t("auth.unlock"), on_click=on_unlock).props(
                                        "dense outline"
                                    )
        ...
        with ui.card().classes("p-4 sm:p-6 bg-white border border-gray-200 rounded-xl shadow-sm"):
            ui.label(t("auth.register_user")).classes(
                "text-base sm:text-lg font-semibold text-gray-800 mb-3 sm:mb-4"
            )
            with ui.column().classes("gap-4 w-full"):
                reg_username = (
                    ui.input(t("auth.username")).props("outlined clearable").classes("w-full")
                )
                reg_email = (
                    ui.input(
                        t("auth.email"), validation={t("auth.email"): lambda v: "@" in (v or "")}
                    )
                    .props("outlined clearable")
                    .classes("w-full")
                )
                reg_password = (
                    ui.input(t("auth.password"))
                    .props("type=password outlined clearable")
                    .classes("w-full")
                )
                ...
                ui.button(t("auth.register"), on_click=register_user, icon="person_add").props(
                    "color=primary"
                ).classes("w-full")
